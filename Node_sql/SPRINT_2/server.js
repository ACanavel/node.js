      ...
      connection.query(transferQuery, [emisor, receptor, monto], (err, result) => {
        if (err) {
          ...
          connection.query(updateQuery1, [monto, emisor], (err) => {
            if (err) {
              console.error('Error al actualizar los balances del emisor:', err);
              connection.rollback(() => {
                res.status(500).json({ error: 'Error al actualizar los balances' });
              });
            } else {
              console.log('Balance del emisor actualizado');

              connection.query(updateQuery2, [monto, receptor], (err) => {
                if (err) {
                  console.error('Error al actualizar los balances del receptor:', err);
                  connection.rollback(() => {
                    res.status(500).json({ error: 'Error al actualizar los balances' });
                  });
                } else {
                  console.log('Balance del receptor actualizado');
